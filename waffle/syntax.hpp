...
// rdt trees
constexpr Node_kind data_tree     = make_tree_node(550);
constexpr Node_kind nil_tree	 = make_tree_node(551);
constexpr Node_kind cons_tree	 = make_tree_node(552);
constexpr Node_kind rid_tree	 = make_tree_node(553);
...
struct RId_tree : Tree {
  RId_tree(const Token* k)
    : Tree(id_tree, k->loc), t1(k) { }
...
struct Nil_tree : Tree {
  Nil_tree(const Token* k)
      : Tree(nil_tree, k->loc),t1(k){ }
...
struct Cons_tree : Tree {
  Cons_tree(const Token* k,Tree* t1, Tree* t2)
    : Tree(cons_tree, k->loc), firstarg(t1), secondarg(t2) { }
    
  Tree* first() const  { return firstarg; }
  Tree* second() const  { return secondarg; }
...
struct Data_tree : Tree {
  Data_tree(const Token* k, Tree* n, Tree* e)
    : Tree(data_tree, k->loc), t1(n), t2(e) { }
