...
init_nodes() {
  ...
  // recursive terms & type
  init_node(data_term, "data");

  init_node(mu_type, "mu-type");
  init_node(cons_type, "cons-type");
}
...
void
pp_nil(std::ostream& os, Nil* t) {
  os << "nil " ;
}

void
pp_cons(std::ostream& os, Cons* t) {
  os << "cons " << group(pretty(t->first()))
  << " " << group(pretty(t->second()));
}
...
pp_expr(std::ostream& os, Node* t) {
  ...
  //case rid_expr: return pp_terminal(os, as<RId>(t));
  // Terms
  ...
  case data_term: return pp_data(os, as<Data>(t));
  ...
  //RDT
  case nil_term: return pp_nil(os, as<Nil>(t));
  case cons_term: return pp_cons(os, as<Cons>(t));
  ...
  case cons_type: return pp_string(os,"Cons");
  case mu_type: return pp_string(os, "Mu");
  default: break;
  }
