...
Name*
elab_name(RId_tree* t) {
  const Token* tok = t->value();
  return new RId(t->loc, tok->text);
}
...
Expr*
elab_data(Data_tree* t) {
  Name* name = elab_name(t->name());
  //std::cout<< pretty(t->name());
  if (not name)
    return nullptr;
  Expr* value = elab_expr(t->value());
  //std::cout<< pretty(t->value());
  if (not value)
    return value;

  // The type is deduced from the value.
  Type* type = get_type(value);
  //std::cout<<pretty(name)<<"\n";
  //std::cout<<pretty(value)<<"\n";
  //std::cout<<pretty(type)<<"\n";
  Data* dt = new Data(t->loc, type, name, value);
  //std::cout<< "after creating data obj\n";
  return declare_data(dt);
}
...
Expr* 
elab_nil(Nil_tree* t){
	Type* t0= get_cons_type();
	return new Nil(t->loc,t0);
}

Expr* 
elab_cons(Cons_tree* t){
	Type* t0= get_mu_type();
	Type* t1= get_cons_type();

	Term* fa=elab_term(t->first());
	Term* sa=elab_term(t->second());
	//std::cout<< pretty(fa)<< "   "<< pretty(sa)<< "\n";
//	std::cout<< pretty(get_type(fa))<< "   "<< pretty(get_type(sa))<< "\n";
	...
	if(not is_same(get_type(fa),get_nat_type())){
     error(fa->loc) << 
         format("term {} does not have type '{}'", typed(get_type(fa)), pretty(get_nat_type()));
         return nullptr;
	}
	if(not is_same(get_cons_type(), get_type(sa))){
     error(sa->loc) << 
         format("term {} does not have type '{}'", typed(get_type(sa)), pretty(get_cons_type()));
         return nullptr;
	}
	//std::cout<< pretty(get_type(fa))<< "   "<< pretty(get_type(sa))<< "\n";
	return new Cons(t->loc,get_cons_type(),fa,sa);
}
...
elab_expr(Tree* t) {
  ...
  case rid_tree: return elab_rid(as<RId_tree>(t));
  case lit_tree: return elab_lit(as<Lit_tree>(t));
  case def_tree: return elab_def(as<Def_tree>(t));
  case data_tree: return elab_data(as<Data_tree>(t));
  ...
  case nil_tree: return elab_nil(as<Nil_tree>(t));
  case cons_tree: return elab_cons(as<Cons_tree>(t));
