...
constexpr Node_kind rid_expr      = make_name_node(1);
// Unit terms
...
//rdt terms and types
constexpr Node_kind nil_term	= make_term_node(600);
constexpr Node_kind cons_term	= make_term_node(601);
constexpr Node_kind mu_type		= make_type_node(650); // the recursive type
constexpr Node_kind cons_type	= make_type_node(651); // the constructor for diff rdt
constexpr Node_kind data_term     = make_term_node(652);
...
// Recursive data type: identifier
struct RId : Name {
  RId(String n)
    : Name(id_expr, nullptr), t1(n) { }
  RId(const Location& l, String n)
    : Name(id_expr, l, nullptr), t1(n) { }

  String t1;
};

// Recursive data type: Nil constuctor
struct Nil : Term {
  Nil(Type* t)
    : Term(nil_term, t) { }
  Nil(const Location& l, Type* t)
    : Term(nil_term, l, t) { }

  //Term* t1;
};

// Recursive data type cons: cons term1 term2
struct Cons : Term {
  Cons(Type* t, Term* t1, Term* t2)
    : Term(cons_term, t), firstarg(t1), secondarg(t2) { }
  Cons(const Location& l, Type* t, Term* t1, Term* t2)
    : Term(cons_term, l, t), firstarg(t1), secondarg(t2) { }

    Term* first() const { return firstarg;}
    Term* second() const {return secondarg;}
...
struct Data : Term {
  Data(Type* t, Name* n, Expr* v)
    : Term(data_term, t), t1(n), t2(v) { }
  Data(const Location& l ,Type* t, Name* n, Expr* v)
    : Term(data_term, l, t), t1(n), t2(v) { }
...
struct Mu_type : Type {
  Mu_type(Type* k) 
    : Type(mu_type, k) { }
  Mu_type(const Location& l, Type* k) 
    : Type(mu_type, l, k) { }
};
...
struct Cons_type : Type {
  Cons_type(Type* k) 
    : Type(cons_type, k) { }
  Cons_type(const Location& l, Type* k) 
    : Type(cons_type, l, k) { }
};
