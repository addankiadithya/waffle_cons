...
/*
Tree* 
parse_nil_expr(Parser& p,Tree *n,const Token *k){
	//if (parse::accept(p, equal_tok)) {
	if(parse::accept(p,nil_tok))
      return new Nil_tree(k);
	//}
   else
      parse::parse_error(p) << "expected 'nil' after '='";
  
  return nullptr;
}

Tree* 
parse_cons_expr(Parser& p,Tree *n,const Token *k){
	//if (parse::accept(p, equal_tok)) {
	if(parse::accept(p, cons_tok))
	{
		if(Tree* fa=parse_num_expr(p))
		{
      		if(Tree* i=parse_rd_def(p)){
      			return new Cons_tree(k, fa,i);
      		}      		
      	}
    	else
	      parse::parse_error(p) << "expected 'num literal' after 'cons'";
	}
 // }
   else
      parse::parse_error(p) << "expected 'nil or cons term' after '='";
...
Tree*
parse_rd_in(Parser& p, const Token* k) {
  Tree* d1 = nullptr;
  if(parse::accept(p, cons_tok))
	{
		//std::cout<< "reaching accept cons token\n";
		if(Tree* fa=parse_num_expr(p))
		{
			//std::cout<< "reaching accept num literal\n";
      		if(Tree* i=parse_rd_in(p,k)){
      			//std::cout<<"reaching cons tree\n";
      			return new Cons_tree(k, fa,i);
      		}   
      		else
	    	{
		      parse::parse_error(p) << "expected 'nil or cons'";
		      return nullptr;
	    	}   		
      	}
    	else
	...
	else if(parse::accept(p,nil_tok)){
			return new Nil_tree(k);
	}
  else 
   		return nullptr;
...
Tree*
parse_rd_def(Parser& p) {
  if (const Token* k = parse::accept(p, data_tok)) 
    if (Tree* n = parse_name(p)) {
    	if (parse::accept(p, equal_tok)) {
		      Tree* d1 = nullptr;
		      if(parse::accept(p, cons_tok))
		      ...
		      if (not d1) {
		        parse::parse_error(p) << "expected 'nil or cons term' after 'name'";
		        return nullptr;
		      }
        	return new Data_tree(k, n, d1);
    	}
    	else
    		parse::parse_error(p)<< "expected '=' token";
  }
  else 
   return nullptr;
}
