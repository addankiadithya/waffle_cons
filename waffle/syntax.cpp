...
init_trees() {
  ...
  init_node(rid_tree, "rid-tree");
  init_node(lit_tree, "lit-tree");
  init_node(def_tree, "def-tree");
  init_node(nil_tree, "nil-tree");
  init_node(cons_tree, "cons-tree");
  init_node(data_tree, "data-tree");
...
bool
is_terminal(Tree* t) {
  return t->kind == id_tree || t->kind == lit_tree || t->kind == rid_tree;
}
...
void
pp_data(std::ostream& os, Data_tree* t) {
  os << "data " << pretty(t->name()) << " = " << pretty(t->value());
}

void
pp_nil(std::ostream& os, Nil_tree* t) {
  os << "nil ";
}

void 
pp_cons(std::ostream& os, Cons_tree* t){
	os<< "cons "<< pretty(t->first())
	<<" "<<pretty(t->second());
}
...
print(std::ostream& os, Tree* t) {
  ...
  case rid_tree: return pp_terminal(os, as<RId_tree>(t));
  ...
  case data_tree: return pp_data(os, as<Data_tree>(t));
  case nil_tree: return pp_nil(os, as<Nil_tree>(t));
  case cons_tree: return pp_cons(os, as<Cons_tree>(t));
  }
